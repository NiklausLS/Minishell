...
/*   Updated: 2024/10/09 22:50:30 by nileempo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fork_and_exec(t_exec *ex, t_token *current, int is_first_cmd, int has_pipe, t_minishell *ms)
{
	pid_t	pid;
	...
	else if (pid == 0)
	{
		...
		child_process(ex, current, is_first_cmd, has_pipe, ms);
	}
	else

...
/*   Updated: 2024/07/09 17:46:04 by nileempo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ 
    ...
    res_pipe = check_pipe(current->data);
    //printf("res_pipe = %d\n", res_pipe);
    //printf("in parse_pipe\n");
    //printf("cmd = %s\n", current->args[0]);
    if (res_pipe == 0)
...
/*static void parse_arg(t_data *data)
{
    if (current)
    {
        if (current->file_type == -1 && current->input_type == -1
                    && current->output_type == -1 && current->pipe_type == -1
                            && current->arg_type == -1 && current->cmd_type == -1)
            current->cmd_type = 1;
        if (current->cmd_type == 1 && current->next)
            current->next->arg_type = 1;
